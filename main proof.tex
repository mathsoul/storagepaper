...
Denote the total amount of the commodity in the storage facility at time $t$ as $Q_t$. Also, denote cumulative amounts of commodity purchased and sold up to time $t$ as $L_t$ and $U_t$, respectively. Here and after, buy and inject have the same meaning, so do sell and withdraw.\\
...
Let the constant buying (selling) costs be $\lambda$ ($\mu$). Since the cost (profit) of buying (selling) one unit at the same price level is the same, the optimal strategy should be bang-bang. Introduce the following two functions of log price $x$,
\begin{equation*}
\begin{split}
...
\begin{enumerate}
  ...
  \item  Keep doing the following steps until either convergence or buying region becomes non-empty, namely $B^n \neq \emptyset$. Here $n$ starts with 0. 
  \begin{enumerate}
  \item Calculate the value function $V^{n}(x,q)$.
  \item If the maximal buying profit is positive, $max_{x}\mathcal{B}V^{n}(x,q)>0$, then set the buying region $B^{n+1} = \{(x,q)|x = x_p^{n+1}(q)\}$, where 
    \begin{equation*}
      x_p^{n+1}(q) =  \arg \max_{x} \mathcal{B}V^{n}(x,q).
    \end{equation*}
  
  \item Else, set the selling region $S^{n+1} = \{(x,q)|x\geq x_s^{n+1}(q)\}$, where 
    \begin{equation*}
  ...
  \item $n = n + 1$.
  \end{enumerate}
\item If previous loop ends when $n = N$ because of buying region becoming non-empty, then keep doing the following  until convergence. Here $n$ starts with $N$.
  \begin{enumerate}
  \item Calculate the value function $V^{n}(x,q)$.
  \item If $n$ is odd, then the buying region remains the same, $B^{n+1} = B^{n}$, and the selling region is updated to $S^{n+1} = \{(x,q)|x\geq x_s^{n+1}(q)\}$.


  \item If $n$ is even, then selling region remains the same, $S^{n+1} = S^{n}$, and buying region is updated to $B^{n+1} = \{(x,q)|x_l^{n+1}(q)\leq x\leq x_u^{n+1}(q)\}$, where
    \begin{equation*}
    \begin{split}
...
Notice that each time only one of selling and buying boundaries moves and both selling region and buying region are increasing. Here and after, the movement of a boundary has the same meaning as the change of the related region. These two facts are critical in the proof of the monotonicity of the value functions $V^{n}$.\\

In order to show this algorithm works, three things need to be proved.
\begin{enumerate}
...
We need two lemmas to prove the monotonicity.

\begin{Lemma}\label{LemmaMaxPr} %maximum principle
If $\LL f = 0$, function $f$ can't achieve the positive maximal or negative minimum at an interior point.
\end{Lemma}
...
Contradiction! We can use the similar argument to prove the negative minimum case.\end{proof}

\begin{Lemma}\label{LemmaMutEx}%mutually exclusive optimal buying and selling policies
At the same log price level $\x$, there won't exist two storage levels $q_1$ and $q_2$ such that selling is optimal at $(\x,q_1)$ while buying is optimal at $(\x,q_2)$.
\end{Lemma}

\begin{proof}
If not, without losing any generality, assume that $(\x,q_1)$ is on the buying boundary and $(\x,q_2)$ is on the selling boundary. Define function $f(x)$ as the buying profit function, $\mathcal{B}V(x,q_1)$. Because $(\x,q_1)$ is on the buying boundary, $f(\x) = 0$. By HJB equation (\ref{HJB}), $f(x)$ achieves its maximum at point $\x$. If function $f$ is differentiable, then $f'(\x) = 0$ because $\x$ is the maximal point. We need to consider two cases.
\begin{enumerate}
  \item Holding region is above $(\x,q_1)$.
  \item Holding region is below $(\x,q_1)$.
\end{enumerate}
Because function $f(x)$ may not be second order continuous, we need to consider the left and right derivative separately. From maximal property and $f'(\x) = 0$, we have $f''(\x+) \leq 0,~f''(\x-) = 0$ for the first case and $f''(\x+)= 0,~f''(\x-) \leq 0$ for the second. In the first case,
\begin{equation*}
...
\begin{equation*}
\begin{split}
\LL f(\x+) = \LL(\mathcal{B}V(\x+,q_1))&= \LL V_q(\x+,q_1) - \LL(e^x + \lambda(q_1))|_{x=\x} \\
...
\frac{1}{2} \sigma^2 e^{\x} + k(\alpha -\x) e^{\x} - \beta (e^{\x} +\lambda(q_1))) = \LL(e^x + \lambda(q_1))|_{x=\x} \geq 0.
\end{equation}
In the second case, we can achieve the same equality using similar derivation above.\\

Define $g(x) = \mathcal{S}V(x,q_2)$. We can replicate the same analysis above to have
...
\end{array}
\end{equation}
Here we also use the monotonicity of $H^n,~S^n$ and $B^n$, that is to say, $H^{n} \supset H^{n+1},~S^{n} \subset S^{n+1}$ and $B^{n} \subset B^{n+1}$. Introduce $\Delta V^{n+1}(x,q) = V^{n+1}(x,q) - V^{n}(x,q)$. From (\ref{Vn}) and (\ref{Vn+1}), we can deduce   
...
\end{split}  
\end{equation}
Because only one of selling and buying region changes, we can further divide it into two cases.
\begin{enumerate}
...
\end{split}  
\end{equation}
In the first case, take derivative with respect to $q$ in the first equality of equation (\ref{dVS}) to have 
...
\begin{enumerate}
  ...
  \end{equation*}
  \item Buy at $(x,Q_{\min})$. By Lemma \ref{LemmaMutEx}, at this log price level $x$, there won't be a selling policy because we already buy at $(x,Q_{\min})$. Especially, we will hold at $(x,Q_{\max})$ because there is no place to inject. Therefore, $\LL \Delta V^{n+1}(x,Q_{\max}) = 0$. Meanwhile, by (\ref{dVqS}),
\begin{equation*}
    \LL \Delta V^{n+1}_q(x,q) = 0 \quad \forall (x,q) \in H^{n+1}\cup B^{n+1}.
\end{equation*}
Since buying and holding are the only policies at $x$, together with above equality,
\begin{equation*}
...
\end{equation*}
This implies $ \Delta V^{n+1}(x,Q_{\min}) = 0$ because $0$ is the only finite function that satisfy the above equation. Therefore,
\begin{equation*}
\begin{split}
...
In the second case, we can use very similar method to have $\Delta V^{n+1}_q \leq 0$ and $\Delta V^{n+1}(x,Q_{\max}) = 0$. Then
\begin{equation*}
\begin{split}
...
\begin{proof} 
Introduce function $f(x) = \Delta V_q^{n+1}(x,q)$ with any fixed storage level $q$. As mentioned before, there are two cases namely only the selling boundary moves and only the buying. In the first case, from previous proof, $f(x)$ are non-negative at all points and achieves its maximum at point $x_s^{n+1}(q)$. Therefore $f'(x_s^{n+1}(q)) > 0$ or  $f'(x_s^{n+1}(q)) = 0$. Because $\LL f(x_s^{n+1}(q)) = 0$, if the latter is true, then we have $0<\beta f(x_s^{n+1}(q)) = \frac{1}{2}\sigma^2 f''(x_s^{n+1}(q))\leq 0$ which is a contradiction. As a result, we proved that $f'(x_s^{n+1}(q)) > 0$.
...
\end{split}
\end{equation}
Because the buying boundary remains, $f(x_u^{n+1}(q)) = f(x_l^{n+1}(q))= 0$. Non-negativity and previous fact implies that $f(x)$ achieves the minimum at $x_u^{n+1}(q)$ and $x_l^{n+1}(q)$. Using similar arguments as $x_s^{n+1}(q)$ to have 
\begin{equation*}
\begin{split}
\left(\mathcal{B}V(x,q)\right)_x|_{x = x_u^{n+1}(q)} & > 0\\
\left(\mathcal{B}V(x,q)\right)_x|_{x = x_l^{n+1}(q)} & < 0\\
\end{split}
\end{equation*}
In the second case that only the buying moves, $f(x)$ still achieves the maximum at $x_s^{n+1}(q)$ and the minimum at $x_u^{n+1}(q)$ and $x_l^{n+1}(q)$. Thus we still have those three strict inequalities hold.
\end{proof}
