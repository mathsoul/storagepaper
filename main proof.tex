...
\begin{enumerate}
  ...
  \item Take positive maximal buying profits points as $B^{n+1}$. If none, $B^{n+1} = \emptyset$.
  ...
  \begin{equation*}
  x^{n+1}(q) = \arg\max_{x}\left(-V_q(x,q) + e^x - \mu(q)\right). 
  \end{equation*}
  ...
  \item $n = n + 1$.
  \end{enumerate}
\item If previous loop ends because of $B^{n} \neq \emptyset$, keep doing the following  until convergence.
  \begin{enumerate}
  \item Solve the fixed boundary problem with $(H^n,S^n,B^n)$ and the solution is denoted as $V^{n}(x,q)$.
  ...
  \begin{equation*}
  \begin{split}
  x_l^{n+1}(q) &= \arg\max_{x<x_l^{n}(q)}\left(V_q(x,q) - e^x - \lambda(q)\right)\\
  x_u^{n+1}(q) &= \arg\max_{x>x_u^{n}(q)}\left(V_q(x,q) - e^x - \lambda(q)\right)
  \end{split}
  \end{equation*}
  \item $H^{n+1} = (S^{n+1} \cup B^{n+1})^c$.
  \item $n = n + 1$. 
...
\subsection{The boundaries can be kept moving}
%
%
%
%
