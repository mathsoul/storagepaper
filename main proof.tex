...
\begin{enumerate}
  \item Find a large enough $M_0$ such that $S^* \supset S^0$ where the selling region $S^0 =  \{(x,q)|x\geq M_0,~Q_{\min} <q \leq Q_{\max}\}$. Holding region $H^0 = (S^0)^c$ and buying region $B^0 = \emptyset$.   
  \item $n$ starts with 0. Keep doing the following steps until either convergence or $B^n \neq \emptyset$, 
  \begin{enumerate}
  ...
  \item If $B^{n+1} = \emptyset$, move down the selling boundary $\partial S^n$ to the maximal selling profitable points to have $S^{n+1}$.
...
\item If previous loop ends because of $B^{n} \neq \emptyset$, keep doing the following  until convergence.
  \begin{enumerate}
  \item Solve the fixed boundary problem with $(H^n,S^n,B^n)$ and the solution is denoted as $V^{n}(x,q)$.
  \item If $n$ is odd, move down the selling boundary $\partial S^n$ to the maximal selling profitable points to have $S^{n+1}$ and $B^{n+1} = B^{n}$.
  \item If $n$ is even, move up and down buying boundary $\partial B^{n+1}$ to the maximal buying profitable points to have $B^{n+1}$ and $S^{n+1} = S^{n}$.
  \item $H^{n+1} = (S^{n+1} \cup B^{n+1})^c$.
  \item $n = n + 1$. 
   \end{enumerate}
\end{enumerate}
...
\begin{enumerate}
  \item Holding at $(x,Q_{\min})$. Directly from the first equality of (\ref{dVS}), we have
  \begin{equation*}
  \LL \Delta V^{n+1}(x,Q_{\min}) = 0
  \end{equation*}
  \item Buying at $(x,Q_{\min})$. By Lemma \ref{LemmaMutEx}, on this log price level $x$, there will only be holding and buying policy. Especially, we will hold at $(x,Q_{\max})$ because there is no place to inject. Therefore,
...
Combine above two observations,
