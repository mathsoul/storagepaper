...
\begin{enumerate}
  \item Find a large enough $x_0$ such that $S^* \supset \{(x,q)|x\geq x_0,~Q_{\min} <q \leq Q_{\max}\}$. The selling region $S^0$ is set to be the latter term and holding region $H^0 = (S^0)^c$. $B^0$ is taken to be $\emptyset$.   
  \item $n$ starts as 0. Keep doing the following steps until either regions get converged or $B^n \neq \emptyset$, 
  \begin{enumerate}
  \item Solve the fixed boundary problem with $(H^n,S^n,B^n)$.
  \item Move down the selling boundary $\partial S^n$ to the maximal selling profitable points to have $S^{n+1}$.
